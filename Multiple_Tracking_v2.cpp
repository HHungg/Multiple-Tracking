//Version 2 bo sung theo gop y cua co: doi tuong nao da duoc gan thi khong gan lai nua trong cung mot frame
...
typedef struct {
	...
	cv::MatND g_hist;
	cv::MatND b_hist;
	int used = 0; // neu da duoc gan thi = 1
} tracking;
...
void processVideo(char* videoFilename) {
	...
	while ((char)keyboard != 'q' && (char)keyboard != 27)
	{
		...
		for (int i = 0; i < tmpvTracking.size(); i++) {
			tmpvTracking[i].used = 0;
		}
		...
		for (int i = 0; i < vTracking.size(); i++) {
			...
			double hist = 0;
			int k = 0; // luu vi tri doi tuong da gan
			for (int j = 0; j < tmpvTracking.size(); j++) {
				if (tmpvTracking[j].used == 0) {
